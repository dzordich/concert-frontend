...
const BannerPlaylist = styled(({ style, playlist }) => (
    ...
    >
        <ImageBackground style={style} source={{ uri: playlist.background }}>
            <Gradient
                colors={['transparent', 'transparent', colors.neutralOpaque5]}
            >
                {playlist.start_date && playlist.end_date && (
                    <Text>
                        {parseAndFormatShortDate(playlist.start_date)} -{' '}
